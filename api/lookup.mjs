const BUILD_ID = "SAB_PUBLIC_MULTI_SOURCE_MASTER_CORPUS_R47_2026_08_23";
...
// =====================================================
// R46 CURRENT-DAY / AUTO-UPDATE LAYER
// - Local lore and confirmed live overrides are checked first.
// - Only genuinely current questions may refresh the S+ /brainrots directory.
// - NVIDIA and Tavily never supply the answer for this layer.
// =====================================================
const R46_DIRECTORY_REFRESH_TTL_MS = 5 * 60 * 1000;
const R46_MIN_DIRECTORY_RECORDS = 500;
const R46_LIVE_OVERRIDES = Object.freeze([
  {
    id: "orchidox-live-riddle-2026-08-22",
    validDate: "2026-08-22",
    subject: "Orchidox",
    answer: "Orchidox24",
    required: ["most op", "brainrot", "today", "my age"],
    sourceNote: "USER_CONFIRMED_LIVE_RIDDLE_AWAITING_SPLUS",
  },
]);

let R46_CURRENT_DIRECTORY = [...R45_SITE_BRAINROTS];
let R46_CURRENT_REFRESHED_AT = 0;
let R46_CURRENT_REFRESH_PROMISE = null;
let R46_CURRENT_REFRESH_ERROR = "";

function r46DateInZone(epoch = Date.now(), zone = env("SAB_TIME_ZONE") || "Pacific/Honolulu") {
  try {
...
function r46BalancedJsonArray(text, marker) {
...
function r46DecodeNextFlight(html) {
  const chunks = [];
  const push = /self\.__next_f\.push\(\[1,("(?:\\.|[^"\\])*")\]\)/g;
  let match;
  while ((match = push.exec(String(html || ""))) !== null) {
    try {
...
function r46ExtractDirectoryFromHtml(html) {
  const decoded = r46DecodeNextFlight(html);
  const candidates = [decoded, String(html || "")];
  for (const text of candidates) {
    for (const marker of ['"initialBrainrots":', "initialBrainrots\\\":"]) {
      const json = r46BalancedJsonArray(text, marker);
      if (!json) continue;
      try {
        const rows = JSON.parse(json).filter((row) =>
          row && row.id && row.name && row.rarity && Number.isFinite(Number(row.baseIncomePerSecond))
        );
        if (rows.length >= R46_MIN_DIRECTORY_RECORDS) return rows;
      } catch {}
    }
  }
  return [];
...
function r46LocalOverrideResolve(question) {
  const q = r43Norm(question);
  const today = r46DateInZone();
  for (const row of R46_LIVE_OVERRIDES) {
    if (row.validDate !== today) continue;
    if (!row.required.every((token) => q.includes(token))) continue;
    return {
      answer: row.answer,
      candidateAnswer: row.answer,
      confidence: 0.999,
      reason: row.sourceNote,
      route: "LOCAL_LIVE_LORE",
      sourceCount: 1,
      sources: [{
        title: `${row.subject} live riddle override`,
        url: "https://steal-a-brainrot.org/brainrots",
        tier: "LOCAL",
        relevance: 1,
      }],
    };
...
function r46LatestReleaseRows(rows, question) {
  const q = r43Norm(question);
  const dated = rows.filter((row) => /^20\d{2}-\d{2}-\d{2}$/.test(String(row.addedAt || "")));
  if (!dated.length) return [];
  const today = r46DateInZone();
  let targetDate = today;
  if (!/\btoday\b/.test(q)) {
    targetDate = dated.map((row) => row.addedAt).sort().at(-1);
  }
  return dated
    .filter((row) => row.addedAt === targetDate && r43Norm(row.releaseStatus || "released") !== "unreleased")
    .sort((a, b) => {
      if (/most op|strongest|best|highest income|earns most|makes most/.test(q)) {
        return Number(b.baseIncomePerSecond || 0) - Number(a.baseIncomePerSecond || 0)
...
async function r46RefreshDirectory(deadline, force = false) {
  if (!force && R46_CURRENT_REFRESHED_AT && nowMs() - R46_CURRENT_REFRESHED_AT < R46_DIRECTORY_REFRESH_TTL_MS) {
    return { rows: R46_CURRENT_DIRECTORY, refreshed: false, cache: "HIT", error: R46_CURRENT_REFRESH_ERROR };
  }
  if (R46_CURRENT_REFRESH_PROMISE) return R46_CURRENT_REFRESH_PROMISE;

  R46_CURRENT_REFRESH_PROMISE = (async () => {
    try {
      const remaining = timeLeft(deadline);
      if (remaining < 350) throw new Error("SPLUS_DIRECTORY_REFRESH_BUDGET_EXHAUSTED");
      const html = await fetchText("SPLUS_DIRECTORY_REFRESH", `${PRIMARY_ORIGIN}/brainrots`, {
        headers: {
          accept: "text/html,application/xhtml+xml",
          "user-agent": "SAB-Lore-Refresh/46",
        },
      }, Math.max(300, Math.min(1800, remaining - 80)));
      const rows = r46ExtractDirectoryFromHtml(html);
      if (rows.length < R46_MIN_DIRECTORY_RECORDS) throw new Error("SPLUS_DIRECTORY_REFRESH_PARSE_MISS");
      R46_CURRENT_DIRECTORY = rows;
      R46_CURRENT_REFRESHED_AT = nowMs();
      R46_CURRENT_REFRESH_ERROR = "";
      return { rows, refreshed: true, cache: "MISS", error: "" };
    } catch (error) {
      R46_CURRENT_REFRESH_ERROR = errorCode(error);
      return { rows: R46_CURRENT_DIRECTORY, refreshed: false, cache: "STALE", error: R46_CURRENT_REFRESH_ERROR };
    } finally {
      R46_CURRENT_REFRESH_PROMISE = null;
    }
  })();
  return R46_CURRENT_REFRESH_PROMISE;
}

async function r46CurrentDirectoryResolve(question, analysis, deadline) {
  const q = r43Norm(question);
  if (!analysis.current || !/brainrot/.test(q)) return null;
  if (!/released|realsed|added|came out|newest|latest|most op|strongest/.test(q)) return null;

  const refresh = await r46RefreshDirectory(deadline);
  const rows = r46LatestReleaseRows(refresh.rows, question);
  if (!rows.length) return null;
  const hit = rows[0];

  let answer = hit.name;
  if (/\bmy age\b/.test(q)) {
    const age = env("SAB_RIDDLE_AGE");
    if (!/^\d{1,3}$/.test(age)) return null;
    answer = `${String(hit.name).replace(/\s+/g, "")}${age}`;
  }

  const result = r41InstantResult(
    answer,
    REL.BRAINROT,
    `/brainrots/${hit.id}`,
    hit.name,
    "R46_AUTO_REFRESH_CURRENT_DAY_OP"
  );
  result.r46Refresh = {
    refreshed: refresh.refreshed,
    cache: refresh.cache,
    error: refresh.error,
    directoryCount: refresh.rows.length,
    targetDate: hit.addedAt,
    rankedBy: "baseIncomePerSecond_desc_then_cost_desc",
  };
  return result;
}
...
// =====================================================
// R47 PUBLIC MULTI-SOURCE CLAIM LAYER
// - Exact R46 remains intact and resolves first.
// - Verified/reported public claims are local and zero-network.
// - Higher tiers win conflicting relation slots deterministically.
// - UNVERIFIED rumor/leak claims are stored in the corpus browser only and
//   can never become normal trivia or Code Sniper answers.
// =====================================================
const R47_PUBLIC_CORPUS = Object.freeze({"build":"R47_PUBLIC_MULTI_SOURCE_CORPUS_2026_08_23","counts":{"sourceFamilies":22,"claims":371,"verifiedOrReportedClaims":368,"unverifiedClaims":3,"media":11,"detectedConflictSlots":1},"sources":[{"id":"official-roblox","name":"Official Roblox experience","tier":"OFFICIAL","trust":1,"host":"roblox.com","url":"https://www.roblox.com/games/109983668079237/Steal-a-Brainrot","coverage":["gameplay","developer","platforms","official-links"],"crawlMode":"html"},{"id":"official-spydersammy-x","name":"SpyderSammy on X","tier":"OFFICIAL","trust":0.99,"host":"x.com","url":"https://x.com/SpyderSammy","coverage":["announcements","events","teasers"],"crawlMode":"public-feed"},{"id":"official-discord","name":"Steal a Brainrot Discord","tier":"OFFICIAL","trust":0.99,"host":"discord.com","url":"https://discord.com/invite/sammy","coverage":["announcements","codes","event-times","public-leaks"],"crawlMode":"public-feed"},{"id":"splus","name":"steal-a-brainrot.org","tier":"S+","trust":0.995,"host":"steal-a-brainrot.org","url":"https://steal-a-brainrot.org","coverage":["brainrots","events","machines","rituals","mutations","codes","rebirth","shops","images"],"crawlMode":"next-flight"},{"id":"fandom","name":"Steal a Brainrot Fandom","tier":"A+","trust":0.97,"host":"stealabrainrot.fandom.com","url":"https://stealabrainrot.fandom.com","coverage":["brainrots","history","trivia","images","community-documentation"],"crawlMode":"robots-blocked-this-run"},{"id":"wiki-b","name":"steal-a-brainrot.wiki","tier":"B","trust":0.94,"host":"steal-a-brainrot.wiki","url":"https://steal-a-brainrot.wiki","coverage":["brainrots","rebirth","events","mutations","images"],"crawlMode":"html"},{"id":"xsteal","name":"xSteal a Brainrot","tier":"C","trust":0.84,"host":"xstealabrainrot.com","url":"https://xstealabrainrot.com","coverage":["brainrots","fusions","rebirth","lucky-blocks","updates"],"crawlMode":"html"},{"id":"pgg","name":"Pro Game Guides","tier":"C","trust":0.86,"host":"progameguides.com","url":"https://progameguides.com/brainrot/","coverage":["machines","events","mechanics","screenshots"],"crawlMode":"html"},{"id":"eldorado","name":"Eldorado Blog","tier":"C","trust":0.8,"host":"eldorado.gg","url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","coverage":["mutations","traits","values","images"],"crawlMode":"html"},{"id":"beebom","name":"Beebom","tier":"C","trust":0.8,"host":"beebom.com","url":"https://beebom.com/steal-a-brainrot-codes/","coverage":["codes","mutations","rebirth","screenshots"],"crawlMode":"html"},{"id":"techwiser","name":"TechWiser","tier":"C","trust":0.79,"host":"techwiser.com","url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","coverage":["gear","shops","rebirth-unlocks","robux","images"],"crawlMode":"html"},{"id":"sportskeeda","name":"Sportskeeda","tier":"C","trust":0.78,"host":"sportskeeda.com","url":"https://www.sportskeeda.com/roblox-news/steal-brainrot-update-5-new-brainrots-traits","coverage":["update-history","rebirth","events"],"crawlMode":"html"},{"id":"gamerant","name":"GameRant","tier":"C","trust":0.75,"host":"gamerant.com","url":"https://gamerant.com/roblox-steal-a-brainrot-all-brainrots-list/","coverage":["brainrots","rarity","cost","income"],"crawlMode":"html"},{"id":"pcgamesn","name":"PCGamesN","tier":"C","trust":0.74,"host":"pcgamesn.com","url":"https://www.pcgamesn.com/steal-a-brainrot/codes","coverage":["codes","current-status"],"crawlMode":"html"},{"id":"playbrainrot","name":"playbrainrot.org","tier":"C","trust":0.72,"host":"playbrainrot.org","url":"https://www.playbrainrot.org/en/updates/","coverage":["update-history","characters","sounds","images"],"crawlMode":"html"},{"id":"field-guide","name":"Steal a Brainrot Field Guide","tier":"C","trust":0.7,"host":"stealabrainrotguides.wiki","url":"https://stealabrainrotguides.wiki","coverage":["codes","brainrots","rebirth","trading","machines","changelog"],"crawlMode":"html"},{"id":"wikioasis","name":"Italian Brainrot WikiOasis","tier":"C","trust":0.68,"host":"italianbrainrot.wikioasis.org","url":"https://italianbrainrot.wikioasis.org/wiki/Steal_a_Brainrot/Brainrot","coverage":["rarities","meme-history","characters"],"crawlMode":"html"},{"id":"timesaver","name":"TimeSaver","tier":"C","trust":0.62,"host":"timesaver.gg","url":"https://timesaver.gg/blog/steal-a-brainrot-value-list","coverage":["trading-values","exist-counts","tier-lists"],"crawlMode":"html"},{"id":"wikipedia","name":"Wikipedia","tier":"REFERENCE","trust":0.72,"host":"wikipedia.org","url":"https://en.wikipedia.org/wiki/Steal_a_Brainrot","coverage":["release-history","records","media","legal-history"],"crawlMode":"html"},{"id":"simple-wikipedia","name":"Simple English Wikipedia","tier":"REFERENCE","trust":0.66,"host":"simple.wikipedia.org","url":"https://simple.wikipedia.org/wiki/Steal_a_Brainrot","coverage":["overview","gameplay","release-date"],"crawlMode":"html"},{"id":"reddit","name":"r/stealabrainrot","tier":"UNVERIFIED","trust":0.42,"host":"reddit.com","url":"https://www.reddit.com/r/stealabrainrot/","coverage":["community-reports","rumors","screenshots","observations"],"crawlMode":"rumor-only"},{"id":"youtube-leaks","name":"Public YouTube leak/news videos","tier":"UNVERIFIED","trust":0.35,"host":"youtube.com","url":"https://www.youtube.com/results?search_query=steal+a+brainrot+leaks","coverage":["public-leaks","rumors","video-evidence"],"crawlMode":"rumor-only"}],"claims":[{"id":"claim-00001","subject":"Steal a Brainrot","relation":"OFFICIAL_DEVELOPER","value":"BRAZILIAN SPYDER","sourceId":"official-roblox","sourceTier":"OFFICIAL","sourceTrust":1,"url":"https://www.roblox.com/games/109983668079237/Steal-a-Brainrot","title":"Official Roblox experience","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00002","subject":"Steal a Brainrot","relation":"ROBLOX_PLACE_ID","value":"109983668079237","sourceId":"official-roblox","sourceTier":"OFFICIAL","sourceTrust":1,"url":"https://www.roblox.com/games/109983668079237/Steal-a-Brainrot","title":"Official Roblox experience","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00003","subject":"Steal a Brainrot","relation":"CORE_ACTION","value":["Buy a Brainrot","Steal Brainrots from other players","Generate money","Rebirth","Buy slaps and troll gear"],"sourceId":"official-roblox","sourceTier":"OFFICIAL","sourceTrust":1,"url":"https://www.roblox.com/games/109983668079237/Steal-a-Brainrot","title":"Official Roblox experience","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00004","subject":"Steal a Brainrot","relation":"SUPPORTED_PLATFORM","value":["Desktop","Mobile","Tablet"],"sourceId":"official-roblox","sourceTier":"OFFICIAL","sourceTrust":1,"url":"https://www.roblox.com/games/109983668079237/Steal-a-Brainrot","title":"Official Roblox experience","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00005","subject":"Steal a Brainrot","relation":"CREDIT","value":["Chris — Nyan Cat permission","@WinnersTakesAll — backpack system"],"sourceId":"official-roblox","sourceTier":"OFFICIAL","sourceTrust":1,"url":"https://www.roblox.com/games/109983668079237/Steal-a-Brainrot","title":"Official Roblox experience","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00006","subject":"Candini Fluffini","relation":"RARITY","value":"Secret","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/candini-fluffini","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00007","subject":"Candini Fluffini","relation":"COST","value":"$14B","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/candini-fluffini","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00008","subject":"Candini Fluffini","relation":"INCOME","value":"$57.5M/s","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/candini-fluffini","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00009","subject":"La Fuse Machine","relation":"RARITY","value":"Secret","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/la-fuse-machine","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00010","subject":"La Fuse Machine","relation":"COST","value":"$35B","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/la-fuse-machine","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00011","subject":"La Fuse Machine","relation":"INCOME","value":"$95M/s","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/la-fuse-machine","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00012","subject":"Yetimatic","relation":"RARITY","value":"Secret","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/yetimatic","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00013","subject":"Yetimatic","relation":"COST","value":"$27.5B","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/yetimatic","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00014","subject":"Yetimatic","relation":"INCOME","value":"$87.5M/s","sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots/yetimatic","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00015","subject":"Steal a Brainrot directory","relation":"BRAINROT_COUNT","value":537,"sourceId":"splus","sourceTier":"S+","sourceTrust":0.995,"url":"https://steal-a-brainrot.org/brainrots","title":"steal-a-brainrot.org","verification":"VERIFIED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00016","subject":"Polaroidini","relation":"RARITY","value":"Secret","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00017","subject":"Polaroidini","relation":"COST","value":"$12.5B","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00018","subject":"Polaroidini","relation":"INCOME","value":"$55M/s","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00019","subject":"Candini Fluffini","relation":"COST","value":"$13B","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":"Conflicts with higher-tier S+ $14B; retained as a provenance claim only."},{"id":"claim-00020","subject":"Candini Fluffini","relation":"INCOME","value":"$57.5M/s","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00021","subject":"La Fuse Machine","relation":"COST","value":"$35B","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00022","subject":"La Fuse Machine","relation":"INCOME","value":"$95M/s","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00023","subject":"Rebirth19","relation":"REWARD","value":["Extra base slot","Grief Shield"],"sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00024","subject":"Grief Shield","relation":"MECHANIC","value":"Blocks one hit from griefers before breaking","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00025","subject":"RNG Machine","relation":"UPDATE_62_CHANGE","value":"Luck boost is temporary during Admin Abuse","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00026","subject":"Lucky Blocks","relation":"UPDATE_62_CHANGE","value":"Timers removed","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00027","subject":"Steal a Brainrot codes","relation":"MECHANIC","value":"Announced by Sammy and may have limited redemption quantity","sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00028","subject":"Steal a Brainrot","relation":"OFFICIAL_TRADING_SYSTEM","value":false,"sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00029","subject":"Steal a Brainrot","relation":"RARITIES","value":["Common","Rare","Epic","Legendary","Mythic","Brainrot God","Secret","OG"],"sourceId":"xsteal","sourceTier":"C","sourceTrust":0.84,"url":"https://xstealabrainrot.com","title":"xSteal a Brainrot","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00030","subject":"RNG Machine","relation":"LOCATION","value":"Middle of the map, right of the Shop","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00031","subject":"RNG Machine","relation":"POOL_SIZE","value":40,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00032","subject":"RNG Machine","relation":"NEW_POOL_ENTRIES_AT_LAUNCH","value":8,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00033","subject":"RNG Machine","relation":"BASE_SPIN_COST","value":"$50K","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00034","subject":"RNG Machine","relation":"UPGRADES","value":["Luck","Mutation Luck","Spin Speed"],"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00035","subject":"RNG Machine","relation":"LUCK_SYMBOL_EFFECT","value":"Rerolls and awards a Secret Brainrot","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00036","subject":"RNG Machine","relation":"PURCHASE_DECISION_TIME","value":"60 seconds","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00037","subject":"RNG Machine","relation":"ROLLED_BRAINROT_STEALABLE","value":true,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00038","subject":"Pi Pi Watermelon","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00039","subject":"Pi Pi Watermelon","relation":"RARITY","value":"Legendary","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00040","subject":"Pi Pi Watermelon","relation":"COST","value":"$315K","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00041","subject":"Pi Pi Watermelon","relation":"INCOME","value":"$1.3K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00042","subject":"Bandito Axolito [Gold]","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00043","subject":"Bandito Axolito [Gold]","relation":"RARITY","value":"Legendary","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00044","subject":"Bandito Axolito [Gold]","relation":"COST","value":"$290K","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00045","subject":"Bandito Axolito [Gold]","relation":"INCOME","value":"$1.5K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00046","subject":"Los Noobinis","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00047","subject":"Los Noobinis","relation":"RARITY","value":"Mythic","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00048","subject":"Los Noobinis","relation":"COST","value":"$4.3M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00049","subject":"Los Noobinis","relation":"INCOME","value":"$12.5K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00050","subject":"Los Noobinis [Gold]","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00051","subject":"Los Noobinis [Gold]","relation":"RARITY","value":"Mythic","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00052","subject":"Los Noobinis [Gold]","relation":"COST","value":"$4.3M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00053","subject":"Los Noobinis [Gold]","relation":"INCOME","value":"$12.5K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00054","subject":"Toiletto Focaccino","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00055","subject":"Toiletto Focaccino","relation":"RARITY","value":"Mythic","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00056","subject":"Toiletto Focaccino","relation":"COST","value":"$4.8M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00057","subject":"Toiletto Focaccino","relation":"INCOME","value":"$16K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00058","subject":"Tic Tic Ribbit","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00059","subject":"Tic Tic Ribbit","relation":"RARITY","value":"Mythic","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00060","subject":"Tic Tic Ribbit","relation":"COST","value":"$6.2M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00061","subject":"Tic Tic Ribbit","relation":"INCOME","value":"$18.7K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00062","subject":"Gattatino Nyanino","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00063","subject":"Gattatino Nyanino","relation":"RARITY","value":"Brainrot God","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00064","subject":"Gattatino Nyanino","relation":"COST","value":"$7.5M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00065","subject":"Gattatino Nyanino","relation":"INCOME","value":"$35K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00066","subject":"Matteo","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00067","subject":"Matteo","relation":"RARITY","value":"Brainrot God","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00068","subject":"Matteo","relation":"COST","value":"$10M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00069","subject":"Ocarlita Orcala","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00070","subject":"Ocarlita Orcala","relation":"RARITY","value":"Brainrot God","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00071","subject":"Ocarlita Orcala","relation":"COST","value":"$45M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00072","subject":"Ocarlita Orcala","relation":"INCOME","value":"$240K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00073","subject":"Espresso Signora","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00074","subject":"Espresso Signora","relation":"RARITY","value":"Brainrot God","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00075","subject":"Espresso Signora","relation":"COST","value":"$25M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00076","subject":"Espresso Signora","relation":"INCOME","value":"$70K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00077","subject":"Pakrahmatmatina","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00078","subject":"Pakrahmatmatina","relation":"RARITY","value":"Brainrot God","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00079","subject":"Pakrahmatmatina","relation":"COST","value":"$40.5M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00080","subject":"Pakrahmatmatina","relation":"INCOME","value":"$225K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00081","subject":"Karkerkar Kurkur","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00082","subject":"Karkerkar Kurkur","relation":"RARITY","value":"Secret","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00083","subject":"Karkerkar Kurkur","relation":"COST","value":"$80M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00084","subject":"Karkerkar Kurkur","relation":"INCOME","value":"$325K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00085","subject":"Chachechi","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00086","subject":"Chachechi","relation":"RARITY","value":"Secret","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00087","subject":"Chachechi","relation":"COST","value":"$85M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00088","subject":"Chachechi","relation":"INCOME","value":"$400K/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00089","subject":"Gelatina Volatina","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00090","subject":"Gelatina Volatina","relation":"RARITY","value":"Secret","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00091","subject":"Gelatina Volatina","relation":"COST","value":"$590M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00092","subject":"Gelatina Volatina","relation":"INCOME","value":"$2.4M/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00093","subject":"Rocketini Frostini","relation":"SOURCE","value":"RNG Machine","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00094","subject":"Rocketini Frostini","relation":"RARITY","value":"Secret","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00095","subject":"Rocketini Frostini","relation":"COST","value":"$700M","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00096","subject":"Rocketini Frostini","relation":"INCOME","value":"$3M/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00097","subject":"Queen Bee event","relation":"FREQUENCY","value":"Every 2 hours","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00098","subject":"Queen Bee event","relation":"DURATION","value":"10 minutes","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00099","subject":"Queen Bee event","relation":"MUTATIONS","value":["Normal Bee","Ice Bee","Fire Bee","Queen Bee"],"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00100","subject":"Queen Bee event","relation":"ESTIMATED_HONEY_PER_EVENT","value":"10–15 Honey jars","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":"Guide estimate, not an official guaranteed amount."},{"id":"claim-00101","subject":"Queen Bee Shop","relation":"RESTOCK","value":"Every 2 hours when a swarm is active","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00102","subject":"Conetto Morsetto","relation":"SHOP","value":"Queen Bee Shop","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00103","subject":"Conetto Morsetto","relation":"HONEY_COST","value":10,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00104","subject":"Conetto Morsetto","relation":"INCOME","value":"$10M/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00105","subject":"Honey Honey Bear","relation":"SHOP","value":"Queen Bee Shop","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00106","subject":"Honey Honey Bear","relation":"HONEY_COST","value":20,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00107","subject":"Honey Honey Bear","relation":"INCOME","value":"$32M/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00108","subject":"Queen Bee","relation":"SHOP","value":"Queen Bee Shop","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00109","subject":"Queen Bee","relation":"HONEY_COST","value":50,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00110","subject":"Queen Bee","relation":"INCOME","value":"$65M/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00111","subject":"S'more Serat","relation":"SHOP","value":"Queen Bee Shop","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00112","subject":"S'more Serat","relation":"HONEY_COST","value":100,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00113","subject":"S'more Serat","relation":"INCOME","value":"$85M/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00114","subject":"Bumbatron","relation":"SHOP","value":"Queen Bee Shop","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00115","subject":"Bumbatron","relation":"HONEY_COST","value":200,"sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00116","subject":"Bumbatron","relation":"INCOME","value":"$172.5M/s","sourceId":"pgg","sourceTier":"C","sourceTrust":0.86,"url":"https://progameguides.com/roblox/steal-a-brainrot-rng-machine-queen-bee-event-guide/","title":"Pro Game Guides","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00117","subject":"Default","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00118","subject":"Default","relation":"MULTIPLIER","value":"1x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00119","subject":"Default","relation":"AVAILABILITY","value":"Regular/daily","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00120","subject":"Gold","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00121","subject":"Gold","relation":"MULTIPLIER","value":"1.25x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00122","subject":"Gold","relation":"AVAILABILITY","value":"Regular/daily","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00123","subject":"Diamond","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00124","subject":"Diamond","relation":"MULTIPLIER","value":"1.5x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00125","subject":"Diamond","relation":"AVAILABILITY","value":"Regular/daily","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00126","subject":"Bloodrot","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00127","subject":"Bloodrot","relation":"MULTIPLIER","value":"2x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00128","subject":"Bloodrot","relation":"AVAILABILITY","value":"Bloodmoon event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00129","subject":"Candy","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00130","subject":"Candy","relation":"MULTIPLIER","value":"4x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00131","subject":"Candy","relation":"AVAILABILITY","value":"Candy Aurora event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00132","subject":"Lava","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00133","subject":"Lava","relation":"MULTIPLIER","value":"6x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00134","subject":"Lava","relation":"AVAILABILITY","value":"Molten event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00135","subject":"Galaxy","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00136","subject":"Galaxy","relation":"MULTIPLIER","value":"7x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00137","subject":"Galaxy","relation":"AVAILABILITY","value":"Galactic event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00138","subject":"Yin Yang","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00139","subject":"Yin Yang","relation":"MULTIPLIER","value":"7.5x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00140","subject":"Yin Yang","relation":"AVAILABILITY","value":"Yin Yang event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00141","subject":"Radioactive","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00142","subject":"Radioactive","relation":"MULTIPLIER","value":"8.5x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00143","subject":"Radioactive","relation":"AVAILABILITY","value":"Radioactive event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00144","subject":"Cursed","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00145","subject":"Cursed","relation":"MULTIPLIER","value":"9x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00146","subject":"Cursed","relation":"AVAILABILITY","value":"Cursed event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00147","subject":"Rainbow","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00148","subject":"Rainbow","relation":"MULTIPLIER","value":"10x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00149","subject":"Rainbow","relation":"AVAILABILITY","value":"Regular/daily","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00150","subject":"Divine","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00151","subject":"Divine","relation":"MULTIPLIER","value":"10x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00152","subject":"Divine","relation":"AVAILABILITY","value":"Divine event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00153","subject":"Cyber","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00154","subject":"Cyber","relation":"MULTIPLIER","value":"11x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00155","subject":"Cyber","relation":"AVAILABILITY","value":"Cyber event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00156","subject":"Phantom","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00157","subject":"Phantom","relation":"MULTIPLIER","value":"12x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00158","subject":"Phantom","relation":"AVAILABILITY","value":"Phantom event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00159","subject":"Crystal","relation":"KIND","value":"Mutation","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00160","subject":"Crystal","relation":"MULTIPLIER","value":"13x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00161","subject":"Crystal","relation":"AVAILABILITY","value":"Crystal event","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00162","subject":"Gold mutation","relation":"NATURAL_SPAWN_CHANCE","value":"10%","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00163","subject":"Diamond mutation","relation":"NATURAL_SPAWN_CHANCE","value":"20.4%","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00164","subject":"Rainbow mutation","relation":"NATURAL_SPAWN_CHANCE","value":"1%","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00165","subject":"Brainrot mutation","relation":"MAX_PER_BRAINROT","value":1,"sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00166","subject":"Traits","relation":"STACK_WITH_MUTATION","value":true,"sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00167","subject":"Brainrot income","relation":"FORMULA","value":"base income × (mutation multiplier + Σ(trait multiplier − 1))","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00168","subject":"Mutation index","relation":"COMPLETION_REQUIREMENT","value":"50%–75% of brainrots in the index","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00169","subject":"Mutation index","relation":"CASH_MULTIPLIER_REWARD","value":"0.5x","sourceId":"eldorado","sourceTier":"C","sourceTrust":0.8,"url":"https://www.eldorado.gg/blog/steal-a-brainrot-mutations-guide/","title":"Eldorado Blog","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00170","subject":"saturn","relation":"KIND","value":"Code","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00171","subject":"saturn","relation":"REWARD","value":"La Vacca Saturno Saturnita","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00172","subject":"saturn","relation":"STATUS","value":"Reported active on August 1, 2026","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00173","subject":"BESTBRAINROTEVER","relation":"KIND","value":"Code","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00174","subject":"BESTBRAINROTEVER","relation":"REWARD","value":"La Vacca Saturno Saturnita","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00175","subject":"BESTBRAINROTEVER","relation":"STATUS","value":"Reported active on August 1, 2026","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00176","subject":"BRUNOISBETTER","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00177","subject":"BLUESAMMY67","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00178","subject":"ILOVEFOOD123","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00179","subject":"SUMMERUPD3","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00180","subject":"NEEDTHIS4CRAFT333","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00181","subject":"OCTOBLOCK555","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00182","subject":"FREEOCTOBLOCK777","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00183","subject":"FREEOCTO555","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00184","subject":"FREEPREMIUMOCTO777","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00185","subject":"WSUMMERUPD1","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00186","subject":"FREEOCTOBASE333W","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00187","subject":"WAVERIDER4FREELOL","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00188","subject":"FREE500DRAGS","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00189","subject":"JOHNPORKDAPIGGY","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00190","subject":"CODESAREREAL321","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00191","subject":"L2EXPLOITERS","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00192","subject":"IMANEGG","relation":"STATUS","value":"Expired","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00193","subject":"Codes UI","relation":"LOCATION","value":"Codes button on the left side of the screen","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00194","subject":"Codes system","relation":"GENERAL_USE_SINCE","value":"June 2026 Summer update","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00195","subject":"Friends","relation":"SERVER_BENEFIT","value":"Each friend grants a passive cash multiplier","sourceId":"beebom","sourceTier":"C","sourceTrust":0.8,"url":"https://beebom.com/steal-a-brainrot-codes/","title":"Beebom","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00196","subject":"Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00197","subject":"Slap","relation":"COINS_SHOP_PRICE","value":"$500","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00198","subject":"Slap","relation":"REBIRTH_REQUIRED","value":0,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00199","subject":"Speed Coil","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00200","subject":"Speed Coil","relation":"COINS_SHOP_PRICE","value":"$750","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00201","subject":"Speed Coil","relation":"REBIRTH_REQUIRED","value":0,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00202","subject":"Trap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00203","subject":"Trap","relation":"COINS_SHOP_PRICE","value":"$1K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00204","subject":"Trap","relation":"REBIRTH_REQUIRED","value":0,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00205","subject":"Iron Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00206","subject":"Iron Slap","relation":"COINS_SHOP_PRICE","value":"$2K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00207","subject":"Iron Slap","relation":"REBIRTH_REQUIRED","value":1,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00208","subject":"Gravity Coil","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00209","subject":"Gravity Coil","relation":"COINS_SHOP_PRICE","value":"$3K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00210","subject":"Gravity Coil","relation":"REBIRTH_REQUIRED","value":1,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00211","subject":"Bee Launcher","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00212","subject":"Bee Launcher","relation":"COINS_SHOP_PRICE","value":"$10K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00213","subject":"Bee Launcher","relation":"REBIRTH_REQUIRED","value":1,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00214","subject":"Gold Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00215","subject":"Gold Slap","relation":"COINS_SHOP_PRICE","value":"$15K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00216","subject":"Gold Slap","relation":"REBIRTH_REQUIRED","value":2,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00217","subject":"Coil Combo","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00218","subject":"Coil Combo","relation":"COINS_SHOP_PRICE","value":"$20K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00219","subject":"Coil Combo","relation":"REBIRTH_REQUIRED","value":2,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00220","subject":"Rage Table","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00221","subject":"Rage Table","relation":"COINS_SHOP_PRICE","value":"$25K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00222","subject":"Rage Table","relation":"REBIRTH_REQUIRED","value":2,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00223","subject":"Diamond Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00224","subject":"Diamond Slap","relation":"COINS_SHOP_PRICE","value":"$50K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00225","subject":"Diamond Slap","relation":"REBIRTH_REQUIRED","value":3,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00226","subject":"Grapple Hook","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00227","subject":"Grapple Hook","relation":"COINS_SHOP_PRICE","value":"$75K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00228","subject":"Grapple Hook","relation":"REBIRTH_REQUIRED","value":3,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00229","subject":"Taser Gun","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00230","subject":"Taser Gun","relation":"COINS_SHOP_PRICE","value":"$100K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00231","subject":"Taser Gun","relation":"REBIRTH_REQUIRED","value":3,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00232","subject":"Emerald Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00233","subject":"Emerald Slap","relation":"COINS_SHOP_PRICE","value":"$200K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00234","subject":"Emerald Slap","relation":"REBIRTH_REQUIRED","value":4,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00235","subject":"Invisibility Cloak","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00236","subject":"Invisibility Cloak","relation":"COINS_SHOP_PRICE","value":"$300K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00237","subject":"Invisibility Cloak","relation":"REBIRTH_REQUIRED","value":4,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00238","subject":"Boogie Bomb","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00239","subject":"Boogie Bomb","relation":"COINS_SHOP_PRICE","value":"$500K","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00240","subject":"Boogie Bomb","relation":"REBIRTH_REQUIRED","value":4,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00241","subject":"Ruby Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00242","subject":"Ruby Slap","relation":"COINS_SHOP_PRICE","value":"$1M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00243","subject":"Ruby Slap","relation":"REBIRTH_REQUIRED","value":5,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00244","subject":"Medusa's Head","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00245","subject":"Medusa's Head","relation":"COINS_SHOP_PRICE","value":"$5M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00246","subject":"Medusa's Head","relation":"REBIRTH_REQUIRED","value":5,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00247","subject":"Dark Matter Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00248","subject":"Dark Matter Slap","relation":"COINS_SHOP_PRICE","value":"$1.5M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00249","subject":"Dark Matter Slap","relation":"REBIRTH_REQUIRED","value":6,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00250","subject":"Web Slinger","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00251","subject":"Web Slinger","relation":"COINS_SHOP_PRICE","value":"$2M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00252","subject":"Web Slinger","relation":"REBIRTH_REQUIRED","value":6,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00253","subject":"Flame Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00254","subject":"Flame Slap","relation":"COINS_SHOP_PRICE","value":"$2.5M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00255","subject":"Flame Slap","relation":"REBIRTH_REQUIRED","value":7,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00256","subject":"Quantum Cloner","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00257","subject":"Quantum Cloner","relation":"COINS_SHOP_PRICE","value":"$3M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00258","subject":"Quantum Cloner","relation":"REBIRTH_REQUIRED","value":7,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00259","subject":"All Seeing Sentry","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00260","subject":"All Seeing Sentry","relation":"COINS_SHOP_PRICE","value":"$5M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00261","subject":"All Seeing Sentry","relation":"REBIRTH_REQUIRED","value":7,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00262","subject":"Nuclear Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00263","subject":"Nuclear Slap","relation":"COINS_SHOP_PRICE","value":"$6M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00264","subject":"Nuclear Slap","relation":"REBIRTH_REQUIRED","value":8,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00265","subject":"Rainbowrath Sword","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00266","subject":"Rainbowrath Sword","relation":"COINS_SHOP_PRICE","value":"$10M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00267","subject":"Rainbowrath Sword","relation":"REBIRTH_REQUIRED","value":8,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00268","subject":"Galaxy Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00269","subject":"Galaxy Slap","relation":"COINS_SHOP_PRICE","value":"$15M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00270","subject":"Galaxy Slap","relation":"REBIRTH_REQUIRED","value":9,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00271","subject":"Laser Cape","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00272","subject":"Laser Cape","relation":"COINS_SHOP_PRICE","value":"$20M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00273","subject":"Laser Cape","relation":"REBIRTH_REQUIRED","value":9,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00274","subject":"Glitched Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00275","subject":"Glitched Slap","relation":"COINS_SHOP_PRICE","value":"$25M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00276","subject":"Glitched Slap","relation":"REBIRTH_REQUIRED","value":10,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00277","subject":"Body Swap Potion","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00278","subject":"Body Swap Potion","relation":"COINS_SHOP_PRICE","value":"$50M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00279","subject":"Body Swap Potion","relation":"REBIRTH_REQUIRED","value":10,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00280","subject":"Splatter Slap","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00281","subject":"Splatter Slap","relation":"COINS_SHOP_PRICE","value":"$75M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00282","subject":"Splatter Slap","relation":"REBIRTH_REQUIRED","value":11,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00283","subject":"Paintball Gun","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00284","subject":"Paintball Gun","relation":"COINS_SHOP_PRICE","value":"$100M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00285","subject":"Paintball Gun","relation":"REBIRTH_REQUIRED","value":11,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00286","subject":"Heart Balloon","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00287","subject":"Heart Balloon","relation":"COINS_SHOP_PRICE","value":"$150M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00288","subject":"Heart Balloon","relation":"REBIRTH_REQUIRED","value":12,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00289","subject":"Magnet","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00290","subject":"Magnet","relation":"COINS_SHOP_PRICE","value":"$200M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00291","subject":"Magnet","relation":"REBIRTH_REQUIRED","value":12,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00292","subject":"Megaphone","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00293","subject":"Megaphone","relation":"COINS_SHOP_PRICE","value":"$250M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00294","subject":"Megaphone","relation":"REBIRTH_REQUIRED","value":13,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00295","subject":"Beehive","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00296","subject":"Beehive","relation":"COINS_SHOP_PRICE","value":"$300M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00297","subject":"Beehive","relation":"REBIRTH_REQUIRED","value":13,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00298","subject":"Gummy Bear","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00299","subject":"Gummy Bear","relation":"COINS_SHOP_PRICE","value":"$400M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00300","subject":"Gummy Bear","relation":"REBIRTH_REQUIRED","value":14,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00301","subject":"Subspace Mine","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00302","subject":"Subspace Mine","relation":"COINS_SHOP_PRICE","value":"$500M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00303","subject":"Subspace Mine","relation":"REBIRTH_REQUIRED","value":14,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00304","subject":"Heatseeker","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00305","subject":"Heatseeker","relation":"COINS_SHOP_PRICE","value":"$750M","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00306","subject":"Heatseeker","relation":"REBIRTH_REQUIRED","value":15,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00307","subject":"Attack Doge","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00308","subject":"Attack Doge","relation":"COINS_SHOP_PRICE","value":"$1B","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00309","subject":"Attack Doge","relation":"REBIRTH_REQUIRED","value":16,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00310","subject":"Giant Potion","relation":"KIND","value":"Gear","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00311","subject":"Giant Potion","relation":"COINS_SHOP_PRICE","value":"$1.5B","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00312","subject":"Giant Potion","relation":"REBIRTH_REQUIRED","value":17,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00313","subject":"Session gear","relation":"PERSISTENCE","value":"Items disappear when the player leaves and must be bought again","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00314","subject":"Blackhole Slap","relation":"ROBUX_PRICE","value":199,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00315","subject":"Flying Carpet","relation":"ROBUX_PRICE","value":375,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00316","subject":"Laser Gun","relation":"ROBUX_PRICE","value":749,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00317","subject":"Ban Hammer","relation":"ROBUX_PRICE","value":1499,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00318","subject":"Witch Broom","relation":"ROBUX_PRICE","value":499,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00319","subject":"Santa's Sleigh","relation":"ROBUX_PRICE","value":1799,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00320","subject":"2x Money gamepass","relation":"ROBUX_PRICE","value":139,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00321","subject":"VIP gamepass","relation":"ROBUX_PRICE","value":2299,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00322","subject":"VIP gamepass","relation":"BENEFIT","value":["0.5x money multiplier","Exclusive VIP tag","+10 seconds base lock time"],"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00323","subject":"Admin Panel gamepass","relation":"ROBUX_PRICE","value":2299,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00324","subject":"Admin Panel gamepass","relation":"BENEFIT","value":"Access to admin commands through ;cmds","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00325","subject":"2x Server Luck","relation":"ROBUX_PRICE","value":249,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00326","subject":"2x Server Luck","relation":"DURATION","value":"15 minutes","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00327","subject":"4x Server Luck","relation":"ROBUX_PRICE","value":999,"sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00328","subject":"4x Server Luck","relation":"DURATION","value":"15 minutes","sourceId":"techwiser","sourceTier":"C","sourceTrust":0.79,"url":"https://techwiser.com/all-items-in-steal-a-brainrot-roblox/","title":"TechWiser","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00329","subject":"Update 57","relation":"DATE","value":"2026-07-11","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00330","subject":"Update 57","relation":"SUMMARY","value":"New Los Traders","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00331","subject":"Update 57","relation":"NEW_BRAINROTS","value":["Los Admins","Moby Bros","Los Sigmas","Los Cornis","Los Tictacs","Los Tangcitos"],"sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00332","subject":"Update 58","relation":"DATE","value":"2026-07-18","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00333","subject":"Update 58","relation":"SUMMARY","value":"Red Carpet and Los Traders additions","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00334","subject":"Update 58","relation":"NEW_BRAINROTS","value":["Noodle Noodle Poodle","Grabatron","Rubiko and Kubiko","Cangurato Gelato"],"sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00335","subject":"Update 59","relation":"DATE","value":"2026-07-25","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00336","subject":"Update 59","relation":"SUMMARY","value":"Crystal Mutation + Crystal Event","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00337","subject":"Update 59","relation":"NEW_BRAINROTS","value":["Toro Españolo","Pizza and Ranch","Examen Bros","Chicleteira Champeona","Los Secret Combinasionas"],"sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00338","subject":"Update 60","relation":"DATE","value":"2026-08-01","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00339","subject":"Update 60","relation":"SUMMARY","value":"New Resume Ritual brainrots","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00340","subject":"Update 60","relation":"NEW_BRAINROTS","value":["Yess my Resume","Noo my Resume"],"sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00341","subject":"Update 61","relation":"DATE","value":"2026-08-08","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00342","subject":"Update 61","relation":"SUMMARY","value":"Bee Event + RNG Machine","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00343","subject":"Update 61","relation":"NEW_BRAINROTS","value":["Bumbatron","La Breakfast Combinasion","Yetimatic","S'more Serat","Queen Bee","Scorpino Coasterino","Honey Honey Bear","Conetto Morsetto","Peschito Machito","Pogo Pogo Penguin","Quackalena"],"sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00344","subject":"Update 62","relation":"DATE","value":"2026-08-15","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00345","subject":"Update 62","relation":"SUMMARY","value":"Rebirth 19 + two RNG Machine Secrets","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00346","subject":"Update 62","relation":"NEW_BRAINROTS","value":["Candini Fluffini","La Fuse Machine"],"sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00347","subject":"Update 57","relation":"LOS_TRADERS_REFRESH","value":"30 minutes","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00348","subject":"Update 62","relation":"BASE_SLOT_AFTER_REBIRTH_19","value":28,"sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00349","subject":"Update 63","relation":"PLANNED_DATE","value":"2026-08-22","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"OUTDATED_REPORT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00350","subject":"Update 63","relation":"PLANNED_CONTENT","value":"TBA; no Secrets announced by the tracker when observed","sourceId":"playbrainrot","sourceTier":"C","sourceTrust":0.72,"url":"https://www.playbrainrot.org/en/updates/","title":"playbrainrot.org","verification":"REPORTED","temporal":"OUTDATED_REPORT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00351","subject":"Rebirth","relation":"RESET_EFFECT","value":["Cash reset","Brainrots lost","Base upgrades reset"],"sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00352","subject":"Rebirth","relation":"PERMANENT_REWARDS","value":["Cash multiplier","Base slots","Lock time","Cash bonus","Coins Shop gear"],"sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00353","subject":"Rebirth","relation":"IN_TRANSIT_EFFECT","value":"Brainrots heading to base return to the Red Carpet","sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00354","subject":"Rebirth","relation":"ONGOING_STEAL_EFFECT","value":"A stolen Brainrot in transit disappears when the owner rebirths","sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00355","subject":"Rebirth 1","relation":"ADDITIONAL_SLOT","value":false,"sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00356","subject":"Rebirth 2","relation":"FLOOR_UNLOCK","value":"Second floor","sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00357","subject":"Rebirth 10","relation":"FLOOR_UNLOCK","value":"Third floor","sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00358","subject":"Rebirth 16","relation":"FIRST_SECRET_REQUIREMENT","value":true,"sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00359","subject":"Rebirth 17","relation":"FIRST_TWO_SECRET_REQUIREMENTS","value":true,"sourceId":"wiki-b","sourceTier":"B","sourceTrust":0.94,"url":"https://steal-a-brainrot.wiki/Rebirth_System","title":"steal-a-brainrot.wiki","verification":"VERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00360","subject":"Steal a Brainrot","relation":"RELEASE_DATE","value":"2025-05-15","sourceId":"simple-wikipedia","sourceTier":"REFERENCE","sourceTrust":0.66,"url":"https://simple.wikipedia.org/wiki/Steal_a_Brainrot","title":"Simple English Wikipedia","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":"Reference claim; other sources may report May 16."},{"id":"claim-00361","subject":"Steal a Brainrot","relation":"DEVELOPER","value":"SpyderSammy","sourceId":"simple-wikipedia","sourceTier":"REFERENCE","sourceTrust":0.66,"url":"https://simple.wikipedia.org/wiki/Steal_a_Brainrot","title":"Simple English Wikipedia","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00362","subject":"Steal a Brainrot","relation":"OWNER","value":"DoBig Studios","sourceId":"simple-wikipedia","sourceTier":"REFERENCE","sourceTrust":0.66,"url":"https://simple.wikipedia.org/wiki/Steal_a_Brainrot","title":"Simple English Wikipedia","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00363","subject":"Steal a Brainrot","relation":"GAMEPLAY_GOAL","value":"Acquire Brainrots primarily by buying or stealing; Brainrots generate money over time","sourceId":"simple-wikipedia","sourceTier":"REFERENCE","sourceTrust":0.66,"url":"https://simple.wikipedia.org/wiki/Steal_a_Brainrot","title":"Simple English Wikipedia","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00364","subject":"Steal a Brainrot","relation":"BASE_INITIAL_SPACES","value":8,"sourceId":"simple-wikipedia","sourceTier":"REFERENCE","sourceTrust":0.66,"url":"https://simple.wikipedia.org/wiki/Steal_a_Brainrot","title":"Simple English Wikipedia","verification":"REPORTED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00365","subject":"Steal a Brainrot","relation":"FILM_ADAPTATION_STATUS","value":"In development with Story Kitchen and DoBig Studios","sourceId":"wikipedia","sourceTier":"REFERENCE","sourceTrust":0.72,"url":"https://en.wikipedia.org/wiki/Steal_a_Brainrot","title":"Wikipedia","verification":"REPORTED","temporal":"CURRENT","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00366","subject":"Bruno Mars event","relation":"PEAK_CONCURRENT_USERS","value":"12.8 million","sourceId":"wikipedia","sourceTier":"REFERENCE","sourceTrust":0.72,"url":"https://en.wikipedia.org/wiki/Steal_a_Brainrot","title":"Wikipedia","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00367","subject":"Bruno Mars event","relation":"COLLECTIBLES_ISSUED","value":"5.4 million","sourceId":"wikipedia","sourceTier":"REFERENCE","sourceTrust":0.72,"url":"https://en.wikipedia.org/wiki/Steal_a_Brainrot","title":"Wikipedia","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00368","subject":"Update 5","relation":"REBIRTH_11_REQUIREMENT","value":["$1T","Odin Din Din Dun"],"sourceId":"sportskeeda","sourceTier":"C","sourceTrust":0.78,"url":"https://www.sportskeeda.com/roblox-news/steal-brainrot-update-5-new-brainrots-traits","title":"Sportskeeda","verification":"REPORTED","temporal":"HISTORICAL","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00369","subject":"World Cup update","relation":"RUMOR","value":"A public video title claimed the World Cup update leaked and asked whether CR7 was coming","sourceId":"youtube-leaks","sourceTier":"UNVERIFIED","sourceTrust":0.35,"url":"https://www.youtube.com/playlist?list=PLym1cBewYjjkqfuEgZEA0UWCMirNP92PC","title":"Public YouTube leak/news videos","verification":"UNVERIFIED","temporal":"RUMOR","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00370","subject":"Unspecified upcoming update","relation":"RUMOR","value":"Public videos claimed every new Brainrot had leaked","sourceId":"youtube-leaks","sourceTier":"UNVERIFIED","sourceTrust":0.35,"url":"https://www.youtube.com/watch?v=yo4J-53Bsw0","title":"Public YouTube leak/news videos","verification":"UNVERIFIED","temporal":"RUMOR","observedAt":"2026-08-23T00:00:00Z","note":""},{"id":"claim-00371","subject":"Steal a Brainrot official Discord","relation":"COMMUNITY_REPORT","value":"Community members report the official Discord link is available from the Roblox experience page","sourceId":"reddit","sourceTier":"UNVERIFIED","sourceTrust":0.42,"url":"https://www.reddit.com/r/stealabrainrot/comments/1n1sgb4/does_anybody_have_a_link_to_the_actual_steal_a/","title":"r/stealabrainrot","verification":"UNVERIFIED","temporal":"STABLE","observedAt":"2026-08-23T00:00:00Z","note":""}],"conflicts":[{"slot":"candini fluffini\u0000cost","variants":[{"value":"14b","claimIds":["claim-00007"]},{"value":"13b","claimIds":["claim-00019"]}]}]});
const R47_TIER_RANK = Object.freeze({ OFFICIAL: 6, "S+": 5, "A+": 4, B: 3, C: 2, REFERENCE: 1, UNVERIFIED: 0 });
const R47_SOURCE_BY_ID = new Map(R47_PUBLIC_CORPUS.sources.map((row) => [row.id, row]));

function r47NormRelation(value) {
  return String(value || "").toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_|_$/g, "");
}

function r47ValueKey(value) {
  return r43Norm(typeof value === "string" ? value : JSON.stringify(value));
}

function r47BuildEligibleClaims() {
  const slots = new Map();
  for (const claim of R47_PUBLIC_CORPUS.claims) {
    if (claim.verification === "UNVERIFIED" || claim.sourceTier === "UNVERIFIED") continue;
    const key = r43Norm(claim.subject) + "\u0000" + r47NormRelation(claim.relation);
  ...
  for (const bucket of slots.values()) {
    const variants = new Set(bucket.map((claim) => r47ValueKey(claim.value)));
    if (variants.size <= 1) {
      eligible.push(...bucket);
      continue;
    }
    const top = Math.max(...bucket.map((claim) => R47_TIER_RANK[claim.sourceTier] || 0));
    eligible.push(...bucket.filter((claim) => (R47_TIER_RANK[claim.sourceTier] || 0) === top));
  }
  return Object.freeze(eligible);
}

const R47_ELIGIBLE_CLAIMS = r47BuildEligibleClaims();

function r47SubjectMatches(question, subject) {
  const q = r43Norm(question);
  const s = r43Norm(subject);
  if (!s) return false;
  if (q.includes(s)) return true;
  if (s.endsWith(" event") && q.includes(s.slice(0, -6))) return true;
  if (s.endsWith(" mutation") && /\b(?:mutation|spawn chance|natural chance|spawn odds)\b/.test(q) && q.includes(s.slice(0, -9))) return true;
  if (s === "queen bee event" && /\bqueen bee\b/.test(q)) return true;
  if (s === "rng machine" && /\brng(?: machine)?\b/.test(q)) return true;
  if (s === "brainrot mutation" && /\bmutations?\b/.test(q)) return true;
  return false;
}

function r47WantedRelations(question) {
  const q = r43Norm(question);
  const out = [];
  const push = (...relations) => { for (const relation of relations) if (!out.includes(relation)) out.push(relation); };
  if (/\bhow many\b.*\bpool\b|\bpool size\b/.test(q)) push("POOL_SIZE");
  if (/\bhow much\b.*\bspin\b|\bspin cost|cost.*spin/.test(q)) push("BASE_SPIN_COST");
  if (/\bspawn chance|natural chance|naturally spawn|spawn odds/.test(q)) push("NATURAL_SPAWN_CHANCE");
  if (/\bwhich rebirth|what rebirth|rebirth required|requires? rebirth/.test(q)) push("REBIRTH_REQUIRED");
  if (/\bhow long|duration|lasts?|last for/.test(q)) push("DURATION");
  if (/\bhow often|frequency|every how|cadence|refresh|restock/.test(q)) push("FREQUENCY", "RESTOCK");
  if (/\bincome|earns?|earning|makes?|per second|a second|\/s/.test(q)) push("INCOME");
  if (/\brarity|tier/.test(q)) push("RARITY");
  if (/\bprice|cost|how much|worth/.test(q)) push("COST", "COINS_SHOP_PRICE", "ROBUX_PRICE", "HONEY_COST", "BASE_SPIN_COST");
  if (/\bwhere|location/.test(q)) push("LOCATION");
  if (/\breward/.test(q)) push("REWARD", "CASH_MULTIPLIER_REWARD");
  if (/\bplatform/.test(q)) push("SUPPORTED_PLATFORM");
  if (/\bdeveloper|made|creator/.test(q)) push("OFFICIAL_DEVELOPER", "DEVELOPER");
  if (/\brelease date|when.*release/.test(q)) push("RELEASE_DATE", "DATE");
  return out;
}

function r47AnswerValue(claim, question) {
  if (claim.relation === "REBIRTH_REQUIRED" && /\brebirth\b/i.test(question)) return `Rebirth${claim.value}`;
  if (Array.isArray(claim.value)) return claim.value.join(", ");
  if (typeof claim.value === "boolean") return claim.value ? "Yes" : "No";
  return String(claim.value);
}

function r47RelationForResult(relation) {
  if (["COST", "COINS_SHOP_PRICE", "ROBUX_PRICE", "HONEY_COST", "BASE_SPIN_COST"].includes(relation)) return REL.COST;
  if (relation === "INCOME") return REL.INCOME;
  if (relation === "RARITY") return REL.RARITY;
  if (relation === "DURATION") return REL.DURATION;
  if (["FREQUENCY", "RESTOCK"].includes(relation)) return REL.FREQUENCY;
  if (["NATURAL_SPAWN_CHANCE", "DROP_RATE"].includes(relation)) return REL.DROP_RATE;
  if (relation === "LOCATION") return REL.LOCATION;
  if (relation === "REBIRTH_REQUIRED") return REL.REBIRTH;
  if (relation.includes("REWARD")) return REL.REWARD;
  if (relation.includes("DATE")) return REL.DATE;
  return REL.LORE;
}

function r47MakeResult(claim, question, reason) {
  const sourceRow = R47_SOURCE_BY_ID.get(claim.sourceId);
  if (!sourceRow) return null;
  const sourceConfig = {
    tier: sourceRow.tier,
    key: `LOCAL_PUBLIC_CORPUS_R47_${sourceRow.tier.replace(/[^A-Z0-9]+/gi, "_")}`,
    host: sourceRow.host,
    confidence: claim.sourceTrust,
  };
  const result = makeResult(
    r47AnswerValue(claim, question),
    r47RelationForResult(claim.relation),
    sourceConfig,
    { title: claim.title, url: claim.url },
    `R47_${reason}_${claim.relation}`,
    claim.sourceTrust
  );
  if (!result) return null;
  result.trustedTier = sourceRow.tier;
  result.trustLog = [`${sourceRow.tier} LOCAL PUBLIC CORPUS • ${result.answer}`];
  result.r47ClaimId = claim.id;
  return result;
}

function r47ReverseSubjectResolve(question) {
  const q = r43Norm(question);
  if (!/\bwhich|what\b/.test(q)) return null;
  const money = r44MoneyValues(question);
  const percent = [...String(question).matchAll(/\b\d+(?:\.\d+)?\s*%/g)].map((m) => r43Norm(m[0]));
  if (!money.length && !percent.length) return null;

  const bySubject = new Map();
  for (const claim of R47_ELIGIBLE_CLAIMS) {
  ...
  for (const [subject, rows] of bySubject) {
    const values = rows.flatMap((row) => r44MoneyValues(typeof row.value === "string" ? row.value : JSON.stringify(row.value)));
    if (money.length && !money.every((clue) => values.includes(clue))) continue;
    const text = r43Norm(rows.map((row) => `${row.relation} ${JSON.stringify(row.value)}`).join(" "));
    if (percent.length && !percent.every((clue) => text.includes(clue))) continue;
    if (/\bsecret\b/.test(q) && !rows.some((row) => row.relation === "RARITY" && r43Norm(row.value) === "secret")) continue;
    hits.push({ subject, rows });
  }
  if (hits.length !== 1) return null;
  const best = [...hits[0].rows].sort((a, b) => (R47_TIER_RANK[b.sourceTier] || 0) - (R47_TIER_RANK[a.sourceTier] || 0))[0];
  return r47MakeResult({ ...best, value: hits[0].subject, relation: "BRAINROT" }, question, "SUBJECT_LEVEL_REVERSE");
}

function r47PublicClaimResolve(question) {
  const q = r43Norm(question);
  if (/\b(?:leak|leaked|rumor|rumoured|rumored|unverified)\b/.test(q)) return null;

  const reverse = r47ReverseSubjectResolve(question);
  if (reverse) return reverse;

  const wanted = r47WantedRelations(question);
  if (!wanted.length) return null;
  const subjects = [...new Set(R47_ELIGIBLE_CLAIMS.filter((claim) => r47SubjectMatches(question, claim.subject)).map((claim) => claim.subject))]
    .sort((a, b) => r43Norm(b).length - r43Norm(a).length);
  if (!subjects.length) return null;
  const topLength = r43Norm(subjects[0]).length;
  const topSubjects = subjects.filter((subject) => r43Norm(subject).length === topLength);
  if (topSubjects.length !== 1) return null;
  const subject = topSubjects[0];
  const rows = R47_ELIGIBLE_CLAIMS.filter((claim) => claim.subject === subject && wanted.includes(r47NormRelation(claim.relation)));
  if (!rows.length) return null;
  rows.sort((a, b) => wanted.indexOf(r47NormRelation(a.relation)) - wanted.indexOf(r47NormRelation(b.relation))
    || (R47_TIER_RANK[b.sourceTier] || 0) - (R47_TIER_RANK[a.sourceTier] || 0));
  const relation = r47NormRelation(rows[0].relation);
  const sameRelation = rows.filter((row) => r47NormRelation(row.relation) === relation);
  const values = new Set(sameRelation.map((row) => r47ValueKey(row.value)));
  if (values.size !== 1) return null;
  return r47MakeResult(sameRelation[0], question, "DIRECT_SUBJECT_RELATION");
}
...
async function resolveQuestion(questionObj, lore = "") {
  ...
  // R46: a dated, explicitly confirmed live-riddle override wins locally while
  // S+ has not yet published the new record. It expires by game-day date.
  const r46Override = r46LocalOverrideResolve(question);
  if (r46Override) {
    const overrideAnalysis = analyzeQuestion(question);
    const final = finalize(r46Override, question, overrideAnalysis, startedAt, {
      instantLoreHit: true,
      currentDayOverride: true,
      networkBypassed: true,
      aiBypassed: true,
      overrideDate: r46DateInZone(),
    });
    final.trustLog = [`LOCAL LIVE LORE • ${r46Override.answer}`];
    final.trustedTier = "LOCAL";
  ...
  // R47: R46/S+ local lore had first refusal. Only now may the inherited
  // verified/reported public claim ledger answer, still with zero network/AI.
  const r47Local = r47PublicClaimResolve(question);
  if (r47Local) {
    const final = finalize(r47Local, question, instantAnalysis, startedAt, {
      publicCorpusR47: true,
      publicCorpusBuildR47: R47_PUBLIC_CORPUS.build,
      publicCorpusClaimIdR47: r47Local.r47ClaimId,
      networkBypassed: true,
      aiBypassed: true,
      unverifiedExcluded: true,
    });
    final.trustLog = r47Local.trustLog;
    final.trustedTier = r47Local.trustedTier;
    final.instantLore = true;
    final.loreLibrary = true;
    setCachedAnswer(question, final);
    return final;
  ...
  // R46: local snapshot missed a genuinely current question. Refresh only the
  // direct S+ directory, rank same-day releases deterministically, then stop.
  // Tavily/NVIDIA are still not allowed to invent this answer.
  const r46DynamicCurrent = await r46CurrentDirectoryResolve(question, instantAnalysis, deadline);
  if (r46DynamicCurrent) {
    const trusted = attachTrustLog(r46DynamicCurrent, SOURCE.PRIMARY);
    const final = finalize(trusted, question, instantAnalysis, startedAt, {
      currentDirectoryRefresh: r46DynamicCurrent.r46Refresh,
      aiQuestionRouter: "BYPASSED_R46_CURRENT_DIRECTORY",
      aiQuestionRouterError: "",
    });
    final.trustLog = trusted.trustLog;
    final.trustedTier = "S+";
...
export async function GET(request) {
  ...
  if (test === "refresh-directory") {
    ...
    const refresh = await r46RefreshDirectory(nowMs() + 2400, true);
    return json(refresh.error ? 502 : 200, {
      ok: !refresh.error,
      build: BUILD_ID,
      test: "R46_SPLUS_DIRECTORY_REFRESH",
      count: refresh.rows.length,
      refreshed: refresh.refreshed,
      cache: refresh.cache,
      error: refresh.error,
      refreshedAt: R46_CURRENT_REFRESHED_AT || null,
    });
  }

  if (test === "r47-synthetic") {
    const syntheticQuestion = "What is the 1% Job Job Job Sahur outcome?";
    const syntheticAnalysis = applyFactSlots(
      syntheticQuestion,
      enforceQuestionSemantics(syntheticQuestion, analyzeQuestion(syntheticQuestion))
    );
    const validSearchRow = {
      title: "Job Job Job Sahur Ritual",
      url: PRIMARY_ORIGIN + "/rituals/job-job-job-sahur-ritual",
      host: SOURCE.PRIMARY.host,
      content: "Noo my Resume has the 1% outcome chance.",
      score: 0.9,
    };
    const mergedWithFailure = mergeSearches([
      { index: 0, query: "variant 0", ok: false, search: { answer: "", results: [], errors: ["TAVILY_VARIANT_FAILED"] } },
      { index: 1, query: "variant 1", ok: true, search: { answer: "", results: [validSearchRow], errors: [] } },
      { index: 2, query: "variant 2", ok: true, search: { answer: "", results: [validSearchRow], errors: [] } },
    ]);
    const goodPage = syntheticPrimaryPage(
      "Job Job Job Sahur Ritual",
      PRIMARY_ORIGIN + "/rituals/job-job-job-sahur-ritual",
      ["Job Job Job Sahur Ritual", "Yess my Resume 99%", "Noo my Resume 1% outcome chance"]
    );
    const isolatedOpens = [
      { ok: false, error: "HTTP_500", row: { title: "bad-1", url: PRIMARY_ORIGIN + "/bad-1" } },
      { ok: false, error: "TIMEOUT", row: { title: "bad-2", url: PRIMARY_ORIGIN + "/bad-2" } },
      { ok: true, error: null, row: validSearchRow, page: goodPage },
    ];
    const survivingPages = isolatedOpens.filter((opened) => opened.ok && opened.page).map((opened) => opened.page);
    const unrelated = syntheticPrimaryPage(
      "Tung Tung Tung Sahur",
      PRIMARY_ORIGIN + "/brainrots/tung-tung-tung-sahur",
      ["Tung Tung Tung Sahur is unrelated to the Job Job Job Sahur ritual."]
    );
    const duplicateQuestion = "What are the 99% and 1% outcomes of the Job Job Job Sahur ritual?";
    const duplicateAnalysis = applyFactSlots(duplicateQuestion, enforceQuestionSemantics(duplicateQuestion, analyzeQuestion(duplicateQuestion)));
    const metadataPage = syntheticPrimaryPage("Metadata", PRIMARY_ORIGIN + "/wiki/metadata", ["requestId ec804a6bfa90408597072080ef2b0063", "random model text"]);
    const candiniCosts = R47_ELIGIBLE_CLAIMS.filter((claim) => claim.subject === "Candini Fluffini" && claim.relation === "COST").map((claim) => claim.value);
    const checks = [
      { name: "one of three Tavily calls fails", pass: mergedWithFailure.results.length === 1 && mergedWithFailure.results[0].queryHits === 2 && mergedWithFailure.errors.includes("TAVILY_VARIANT_FAILED") },
      { name: "two page fetches fail while one succeeds", pass: isolatedOpens.filter((opened) => !opened.ok).length === 2 && survivingPages.length === 1 },
      { name: "unrelated similar pages cannot win", pass: aggressivePageUsable(syntheticQuestion, syntheticAnalysis, unrelated, SOURCE.PRIMARY).usable === false },
      { name: "duplicate relation slots remain separate", pass: duplicateAnalysis.factSlots.length === 2 && duplicateAnalysis.factSlots.every((slot) => slot.relation === REL.OUTCOME) && duplicateAnalysis.factSlots[0].qualifier !== duplicateAnalysis.factSlots[1].qualifier },
      { name: "hashes request ids metadata random AI cannot become final", pass: loreAnswerSafe("ec804a6bfa90408597072080ef2b0063", { relation: REL.LORE }) === false && validateBundleAnswer("What is the ritual outcome?", { relation: REL.OUTCOME }, "Completely Invented Brainrot", [metadataPage], buildEvidenceBundle("What is the ritual outcome?", { relation: REL.OUTCOME }, [metadataPage])).valid === false },
      { name: "lower conflicting source cannot override S+", pass: candiniCosts.length === 1 && candiniCosts[0] === "$14B" },
      { name: "unverified claims excluded from answer index", pass: R47_ELIGIBLE_CLAIMS.every((claim) => claim.verification !== "UNVERIFIED" && claim.sourceTier !== "UNVERIFIED") },
    ];
    return json(checks.every((row) => row.pass) ? 200 : 500, {
      ok: checks.every((row) => row.pass),
      build: BUILD_ID,
      test: "R47_SYNTHETIC_FAILURE_AND_SAFETY",
      checks,
    });
  }
  ...
  return json(200, {
    ...
    architecture: {
      ...
      publicCorpusSourcesR47: R47_PUBLIC_CORPUS.counts.sourceFamilies,
      publicCorpusClaimsR47: R47_PUBLIC_CORPUS.counts.claims,
      publicCorpusEligibleClaimsR47: R47_ELIGIBLE_CLAIMS.length,
      publicCorpusConflictsR47: R47_PUBLIC_CORPUS.counts.detectedConflictSlots,
      unverifiedMayAnswerNormalTriviaR47: false,
      autoUpdateDirectoryR46: true,
      currentDayRankingR46: "income_desc_then_cost_desc",
      currentDayRefreshTtlMsR46: R46_DIRECTORY_REFRESH_TTL_MS,
      liveOverrideCountR46: R46_LIVE_OVERRIDES.length,
